  ...
  if (showMin) {
    ...
    if (crossings && crossings.length) {
      for (const c of crossings) {
        const xPlot = toViewerPlotTimestamp(new Date(c.ms).toISOString());
        shapes.push({
          type: "line",
          xref: "x",
          x0: xPlot,
          ...
          yref: "y",
          y0: minValue,
          y1: minValue + (c.direction === "up" ? 0.35 : -0.35),
          line: {
            color: c.direction === "up" ? "rgba(52,199,89,.75)" : "rgba(255,69,58,.75)",
            width: 1.5,
            dash: "dot",
          },
        });
        annotations.push({
          ...
          yref: "y",
          y: minValue,
          yshift: c.direction === "up" ? 18 : -18,
          showarrow: false,
          text: `${c.direction === "up" ? "▲" : "▼"} ${formatShortLocal(c.ms)}`,
          font: {
            family: '-apple-system, BlinkMacSystemFont, "SF Pro Display", sans-serif',
            size: 10,
            color: c.direction === "up" ? "#1B8A3E" : "#C0392B",
          },
...
function initThresholdControls() {
  ...
  function onChange() {
    ...
    applyThresholds();
    if (typeof lastPayload !== "undefined" && lastPayload) updateCycleCards(lastPayload);
  }
...
function buildTraces(data) {
  ...
  if (lastCrossings.length) {
    ...
    traces.push({
      ...
      textfont: { family: "-apple-system", size: 10, color: "#6e6e73" },
      marker: {
        size: 10,
...
function setupChartHover(gd) {
  ...
  gd.on("plotly_hover", (event) => {
    ...
    // Prefer water-level / predicted point over raw markers / crossings
    let pt = event.points.find((p) => p.data && (p.data.name === "Water Level" || p.data.name === "Predicted"));
    if (!pt) pt = event.points[0];
    if (pt && typeof pt.y === "number" && Number.isFinite(pt.y)) {
      isHoveringChart = true;
      hoverReadoutValue = pt.y;
      if (readout) {
        readout.innerHTML = `${pt.y.toFixed(2)}<span class="unit">ft</span>`;
      }
      const { minValue, maxValue } = getEffectiveThresholds();
      const nearHigh = maxValue - pt.y <= WARNING_MARGIN_FT;
      const nearLow = pt.y - minValue <= WARNING_MARGIN_FT;
      if (readout) {
        readout.classList.toggle("near-limit-high", nearHigh && !nearLow);
        readout.classList.toggle("near-limit-low", nearLow);
      }
      if (readoutMeta && pt.x != null) {
        // Show the time under the cursor while hovering
        const xLabel = typeof pt.x === "string" ? pt.x : String(pt.x);
        readoutMeta.textContent = `At ${formatHoverAxisTime(pt.x)}`;
      }
    }
  });
  ...
  // Also clear on mouse leave of the chart container
...
function formatHoverAxisTime(xVal) {
  // Plotly may give a date string already in viewer-local plot format
  try {
